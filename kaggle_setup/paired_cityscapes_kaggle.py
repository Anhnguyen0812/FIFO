...
class Pairedcityscapes(data.Dataset):
    ...
    def __init__(self, src_root, trg_root, src_list_path, trg_list_path, max_iters=None, mean=(128, 128, 128), ignore_label=255, set='train'):
        """
        Args:
            src_root: Root path của foggy images (/kaggle/input/cityscapes-filtered-fog)
            trg_root: Root path của clear images (/kaggle/input/cityscapes-filtered-fog)
            src_list_path: File list của foggy images
            trg_list_path: File list của clear images
        """
        ...
        # Parse file paths theo cấu trúc Kaggle
        for name in self.img_ids:
            # Foggy image path
            # name format: aachen/aachen_000000_000019_leftImg8bit_foggy_beta_0.005.png
            src_img_file = osp.join(
                self.src_root, 
                "foggy_filtered/foggy_data/leftImg8bit_foggy/%s/%s" % (self.set, name)
            )
            
            # Clear image path - convert name
            # From: aachen/aachen_000000_000019_leftImg8bit_foggy_beta_0.005.png
            # To:   aachen/aachen_000000_000019_leftImg8bit.png
            ...
            # Label path
            # From: aachen/aachen_000000_000019_leftImg8bit_foggy_beta_0.005.png
            # To:   aachen/aachen_000000_000019_gtFine_labelIds.png
            label_name = name.replace('_leftImg8bit_foggy_beta_0.005.png', '_gtFine_labelIds.png')
            label_file = osp.join(
                self.src_root,
                "gtFine_filtered/gtFine_data/gtFine/%s/%s" % (self.set, label_name)
            )
