    ...
    for i_iter in tqdm(range(start_iter, args.num_steps)): 
        ...
        for sub_i in range(args.iter_size):
            ...
            for idx, layer in enumerate(fsm_weights):
                ...
                # Dynamic concatenation based on actual batch size
                fog_factor_list = []
                fog_label_list = []
                for batch_idx in range(args.batch_size):
                    ...
                    fog_label_list.extend([0, 1, 2])  # SF=0, CW=1, RF=2
                
                fog_factor_embeddings = torch.cat(fog_factor_list, 0)
                fog_factor_labels = torch.LongTensor(fog_label_list)
                ...
                fog_factor_embeddings = fog_factor_embeddings.div(
                    fog_factor_embeddings_norm.expand(size_fog_factor[1], size_fog_factor[0]).t()
                )
